...
suite('lifecycle', () => {
    ...
    suite('Processing files results in changes', () => {
        ...
        stubs.beforeEach(ctx => {
            ...
            ctx.stubbedReadFile.withArgs('<foo-path>', 'utf8').resolves(JSON.stringify(fakeFooProjectJson));
            ctx.stubbedReadFile.withArgs('<bar-path>', 'utf8').resolves(JSON.stringify(fakeBarProjectJson));
        ...
        stubs.test('Writes updated files', async ctx => {
            ...
            ctx.stubbedNormalizer.normalizeOptions.withArgs(mockOptions, mockContext).returns(options);
    
            ctx.mockedProcessor.withArgs(match({
                nxJson: fakeNxJson,
                projectJsons: [fakeFooProjectJson, fakeBarProjectJson],
                options,
            })).returns({
                processedNxJson: fakeProcessedNxJson,
                processedProjectJsons: [fakeProcessedFooProjectJson, fakeProcessedBarProjectJson],
            });
    
            ctx.stubbedWriteFile.withArgs('<nx-json-path>', JSON.stringify(fakeProcessedNxJson, null, 2), 'utf8').resolves();
            ctx.stubbedWriteFile.withArgs('<foo-path>', JSON.stringify(fakeProcessedFooProjectJson, null, 2), 'utf8').resolves();
            ctx.stubbedWriteFile.withArgs('<bar-path>', JSON.stringify(fakeProcessedBarProjectJson, null, 2), 'utf8').resolves();
    
            await ctx.lifecycle.lifecycle(mockOptions, mockContext);

            expect(ctx.stubbedWriteFile.calledWith('<nx-json-path>', JSON.stringify(fakeProcessedNxJson, null, 2), 'utf8')).to.equal(true);
            expect(ctx.stubbedWriteFile.calledWith('<foo-path>', JSON.stringify(fakeProcessedFooProjectJson, null, 2), 'utf8')).to.equal(true);
            expect(ctx.stubbedWriteFile.calledWith('<bar-path>', JSON.stringify(fakeProcessedBarProjectJson, null, 2), 'utf8')).to.equal(true);
        });
        ...
        stubs.test('Check reports a failure', async ctx => {
            ...
            await expect(ctx.lifecycle.lifecycle(mockOptions, mockContext)).eventually.be.rejectedWith(Error).that.contain({
