...
import { AsyncContainer, createAsyncContainer, createSyncContainer, SyncContainer } from '#container';
import { type GenericHaystackId, type HaystackId, type HaystackIdType, type StripAnnotations } from '#identifier';
import type { Extendable, InvalidInput, NonExtendable } from '#types';
import { HaystackDuplicateOutputError } from '#errors';

type ExpandOutputUndefinable<T, Named extends string | symbol | null, Undefinable extends boolean> =
    true extends Undefinable
    ? [NonExtendable<T | undefined, Named>]
    : [NonExtendable<T | undefined, Named>] | [NonExtendable<T, Named>];
type ExpandOutputNullable<T, Named extends string | symbol | null, Nullable extends boolean, Undefinable extends boolean> =
    true extends Nullable
    ? ExpandOutputUndefinable<T | null, Named, Undefinable>
    : ExpandOutputUndefinable<T | null, Named, Undefinable> | ExpandOutputUndefinable<T, Named, Undefinable>;
type BindingOutputType<T extends GenericHaystackId> = T extends HaystackId<infer BaseType, any, infer Named, infer Nullable, infer Undefinable, boolean | 'async', boolean>
    ? ExpandOutputNullable<BaseType, Named, Nullable, Undefinable>
    : never;

type SimplifyDependencyType<T extends readonly GenericHaystackId[]> = {
    [Index in keyof T]: [NonExtendable<StripAnnotations<HaystackIdType<T[Index]>, 'latebinding' | 'supplier'>, T[Index]['annotations']['named']>]
}[number];

export type GenericModule = Module<[Extendable], [Extendable], boolean>;

type ModuleOutputs<T extends GenericModule> = T extends Module<infer O, [Extendable], boolean> ? O : never;
type ModuleDependencies<T extends GenericModule> = T extends Module<[Extendable], infer D, boolean> ? D : never;
...
> {
    ...
    public static fromBinding<T extends GenericBinding>(binding: T): Module<
    ...
    public addBinding<
        T extends GenericBinding
    >(
        binding: T,
        ...invalidInput: true extends (Outputs extends BindingOutputType<T['outputId']> ? true : false) ? [1] : []
    ): Module<
    ...
    >(...[binding]: [
        T, 
        ...(true extends (Outputs extends BindingOutputType<T['outputId']> ? true : false) ? [1] : [])
    ]): Module<
        Outputs | BindingOutputType<T['outputId']>,
        Dependencies | SimplifyDependencyType<T['depIds']>,
        T['isAsync'] extends true ? true : Async
    ...
    public mergeModule<
        T extends GenericModule
    >(
        module: T,
        ...invalidInput: true extends (Outputs extends ModuleOutputs<T> ? true : false) ? [1] : []
    ): Module<Outputs | ModuleOutputs<T>, Dependencies | ModuleDependencies<T>, T['isAsync'] extends true ? true : Async>;
    public mergeModule<
        T extends GenericModule
    >(...[module]: [
        T, 
        ...
        ) as Module<Outputs | ModuleOutputs<T>, Dependencies | ModuleDependencies<T>, T['isAsync'] extends true ? true : Async>;
    };

    public static createContainer<T extends  Module<[Extendable], [Extendable], true>>(
        module: T, 
        ...invalidInput: [ModuleDependencies<T>] extends [ModuleOutputs<T>] ? [] : [InvalidInput<'MissingOutput'>]
