        ...
        const isChainSafe = (chain: GenericHaystackId[]): boolean => {
            if (
                ...
                return bindingChain.every(
                    ({ dependencyId }) => !dependencyId.annotations.supplier
                );
            }
            return false;
        };
    ...
    #checkForAsyncSupplier() {
        const isSafeForSync = (
            ...
            return binding.dependencyIds.every(dependencyId => {
                const { supplier } = dependencyId.annotations;
                if (supplier && (!supplier.propagateScope || !supplier.sync)) {
        ...
        for (const [propagateScope, isSupplierScope, optimisticScopes] of [
            ...
            for (const syncBinding of syncBindings) {
                if (isSafeForSync(syncBinding, new Set(), optimisticScopes)) {
    ...
    #wireSingletons(): void {
        const collectBindings = (
            ...
            for (const dependencyId of binding.dependencyIds) {
                ...
                if (lateBinding || (supplier && !supplier.sync)) {
                    continue;
                }
        ...
        for (const binding of this.#bindings.filter(
            binding => binding.scope === optimisticSingletonScope
        )) {
                ...
                binding.dependencyIds.map(
                    async (dependencyId): Promise<readonly [unknown]> => {
                        ...
                        if (supplier) {
                            ...
                            const dependency =
                                this.#isAsyncImplementationMap.get(
                                    dependencyBinding
                                )!
    ...
    async preload(): Promise<void> {
        ...
        this.#preloaded = Promise.resolve()
            .then(async () => {
                ...
                await Promise.all(
                    [...this.#singletonMap.keys()].map(binding =>
