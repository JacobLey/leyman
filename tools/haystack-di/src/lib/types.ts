...
export type InstanceOfClass<T extends IsClass> = InstanceType<{ new (): never } & T>;
...
export type LiteralStringType<T extends string | symbol> = ToTuple<T> extends { length: 1 }
    ? (
        string extends T
            ? LiteralStringInput
            : (
