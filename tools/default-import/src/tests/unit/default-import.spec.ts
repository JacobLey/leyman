...
const [dynamicCjs, dynamicEsm, dynamicNamedCjs, dynamicNoDefault, dynamicRootCjs] =
    await Promise.all([
        import('../data/cjs.cjs'),
        import('../data/esm.js'),
...
suite('defaultImport', () => {
    suite('Returns default', () => {
        test('ESM export', () => {
            ...
            expectTypeOf(DefaultImport.defaultImport(esm)).toEqualTypeOf<{
                esm: boolean;
            }>();

            expect(DefaultImport.defaultImport(DefaultImport.defaultImport(esm))).to.eq(
                DefaultImport.defaultImport(esm)
            );
            expectTypeOf(
                DefaultImport.defaultImport(DefaultImport.defaultImport(esm))
            ).toEqualTypeOf<{ esm: boolean }>();

            expect(DefaultImport.defaultImport(esm)).to.deep.equal({
                esm: true,
            });
        });
        ...
        test('Only default export', () => {
            ...
            expectTypeOf(DefaultImport.defaultImport(cjs)).toEqualTypeOf<{
                cjs: boolean;
            }>();

            expect(DefaultImport.defaultImport(cjs)).to.eq(DefaultImport.defaultImport(dynamicCjs));
            expect(DefaultImport.defaultImport(cjs)).to.eq(
                DefaultImport.defaultImport(dynamicCjs.default)
            );

            expectTypeOf(DefaultImport.defaultImport(dynamicCjs)).toEqualTypeOf<{ cjs: boolean }>();
            expectTypeOf(DefaultImport.defaultImport(dynamicCjs.default)).toEqualTypeOf<{
                cjs: boolean;
            }>();

            expect(DefaultImport.defaultImport(DefaultImport.defaultImport(cjs))).to.eq(
                DefaultImport.defaultImport(cjs)
            );
            expectTypeOf(
                DefaultImport.defaultImport(DefaultImport.defaultImport(cjs))
            ).toEqualTypeOf<{ cjs: boolean }>();

            expect(DefaultImport.defaultImport(cjs)).to.deep.equal({
                cjs: true,
            });
        });
    });
    ...
    suite('Returns raw', () => {
        test('Export is single object', () => {
            ...
            expectTypeOf(DefaultImport.defaultImport(rootCjs)).toEqualTypeOf<{
                rootCjs: boolean;
            }>();

            expect(DefaultImport.defaultImport(rootCjs)).to.eq(
                DefaultImport.defaultImport(dynamicRootCjs)
            );
            expectTypeOf(DefaultImport.defaultImport(dynamicRootCjs)).toEqualTypeOf<{
                rootCjs: boolean;
            }>();
            expectTypeOf(DefaultImport.defaultImport(dynamicRootCjs.default)).toEqualTypeOf<{
                rootCjs: boolean;
            }>();

            expect(DefaultImport.defaultImport(DefaultImport.defaultImport(rootCjs))).to.eq(
                DefaultImport.defaultImport(rootCjs)
            );
            expectTypeOf(
                DefaultImport.defaultImport(DefaultImport.defaultImport(rootCjs))
            ).toEqualTypeOf<{ rootCjs: boolean }>();

            expect(DefaultImport.defaultImport(rootCjs)).to.deep.equal({
                rootCjs: true,
            });
        });
        ...
        test('No default is found', () => {
            ...
            expectTypeOf(DefaultImport.defaultImport(noDefault)).toEqualTypeOf<{
                readonly noDefault: true;
            }>();

            expect(DefaultImport.defaultImport(noDefault)).to.eq(
                DefaultImport.defaultImport(dynamicNoDefault)
            );
            expectTypeOf(DefaultImport.defaultImport(dynamicNoDefault)).toEqualTypeOf<{
                readonly noDefault: true;
            }>();

            expect(DefaultImport.defaultImport(DefaultImport.defaultImport(noDefault))).to.eq(
            ...
            expectTypeOf(DefaultImport.defaultImport(namedCjs)).toEqualTypeOf<{
                readonly named: '<named>';
            }>();

            expect(DefaultImport.defaultImport(namedCjs)).to.eq(
                DefaultImport.defaultImport(dynamicNamedCjs)
            );
            expectTypeOf(DefaultImport.defaultImport(dynamicNamedCjs)).toEqualTypeOf<{
                readonly named: '<named>';
            }>();

            expect(DefaultImport.defaultImport(DefaultImport.defaultImport(namedCjs))).to.eq(
            ...
            expect(DefaultImport.defaultImport(namedCjs)).to.deep.equal({
                named: '<named>',
            });
        });
    });
