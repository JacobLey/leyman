...
suite('defaultImport', () => {
    suite('Returns default', () => {
        test('ESM export', () => {
            ...
            expect(DefaultImport.defaultImport(esm)).to.eq(
                DefaultImport.defaultImport(dynamicEsm)
            );
        ...
        test('Only default export', async () => {
            ...
            expect(DefaultImport.defaultImport(cjs)).to.eq(
                DefaultImport.defaultImport(dynamicCjs)
            );
            expect(DefaultImport.defaultImport(cjs)).to.eq(
    ...
    suite('Returns raw', () => {
        test('Export is single object', () => {
            ...
            expectTypeOf(
                DefaultImport.defaultImport(dynamicRootCjs.default)
            ).toEqualTypeOf<{ rootCjs: boolean }>();
    ...
    test('Handles literals', () => {
        ...
        expectTypeOf(DefaultImport.defaultImport(123 as const)).toEqualTypeOf(
            123 as const
        );
        ...
        expectTypeOf(DefaultImport.defaultImport(123n as const)).toEqualTypeOf(
            123n as const
        );

        expect(DefaultImport.defaultImport('<abc>')).to.equal('<abc>');
        expectTypeOf(DefaultImport.defaultImport('<abc>')).toEqualTypeOf(
            '<abc>'
        );

        expect(DefaultImport.defaultImport(true)).to.equal(true);
        expectTypeOf(
            DefaultImport.defaultImport(true)
        ).toEqualTypeOf<boolean>();

        expect(DefaultImport.defaultImport(false)).to.equal(false);
        expectTypeOf(DefaultImport.defaultImport(false as const)).toEqualTypeOf(
            false
        );

        expect(DefaultImport.defaultImport([])).to.deep.equal([]);
        expectTypeOf(DefaultImport.defaultImport([] as const)).toEqualTypeOf(
            [] as const
        );
