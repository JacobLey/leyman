...
suite('commonProxy', async () => {
    ...
    test('Handles methods', async () => {
        ...
        expectTypeOf(proxiedMethods.addAllNums).toEqualTypeOf<(...nums: number[]) => Promise<number>>();
        ...
        expectTypeOf(proxiedMethods.delayForLongestTime).toEqualTypeOf<(...nums: number[]) => Promise<void>>();
    ...
    test('Dynamic method', async () => {
        ...
        expectTypeOf(proxiedPromise).toEqualTypeOf<(a: number, ...rest: string[]) => Promise<string>>();
