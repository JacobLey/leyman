...
import type { Context as MochaContext, Done, HookFunction, TestFunction } from 'mocha';
...
suite('Failure cases', () => {
    ...
    const defaultBaseTest = (title: string, cb: Mocha.Func | Mocha.AsyncFunc) => {
        cb.call({
            test: fakeCurrentTest,
        } as MochaContext, () => {});
        return {};
    };
    const defaultTest = Object.assign(defaultBaseTest, { skip: defaultBaseTest, only: defaultBaseTest }) as TestFunction;
    ...
    suite('Test failure', () => {
        ...
        const beforeModule = before(() => {
            ...
            const defaultHooksModule = createModule(
                ...
                bind(afterEachIdentifier).withInstance(defaultHook)
            ).addBinding(
                bind(afterIdentifier).withInstance(defaultHook)
            );
        ...
        beforeModule.test('Test with done returns truthy', function(ctx, done) {

            // Trick lock into thinking test is complete
            this.runnable().duration = -1;

            const stubDone = stub();
            const fakeBaseTest = (title: string, cb: Mocha.Func | Mocha.AsyncFunc) => {
            ...
            const fakeTest = createContainer(
                ctx.module.addBinding(
                    bind(testIdentifier).withInstance(Object.assign(fakeBaseTest, { skip: fakeBaseTest, only: fakeBaseTest }) as TestFunction)
                )
        ...
        beforeModule.test('Test with done throws an error', function(ctx, done) {
        ...
        test('Test declared inside a test', () => {
            ...
            expect(() => test('This will never run', () => {})).to.throw(Error).that.contains({
    ...
    suite('Hooks failure', () => {
        ...
        beforeModule.test('One-time hook with done throws an error', function(ctx, done) {
            ...
            const before = createContainer(
                ctx.module.addBinding(
                    bind(beforeIdentifier).withInstance(fakeHook as HookFunction)
                ).addBinding(
                    bind(beforeEachIdentifier).withInstance(defaultHook)
                )
        ...
        beforeModule.test('Per-test hook with done throws an error', function(ctx, done) {
            ...
            const beforeEach = createContainer(
                ctx.module.addBinding(
                    bind(beforeEachIdentifier).withInstance(fakeHook as HookFunction)
                ).addBinding(
                    bind(beforeIdentifier).withInstance(defaultHook)
                )
