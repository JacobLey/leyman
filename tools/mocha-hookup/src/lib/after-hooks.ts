...
export interface AfterChain<ExistingContext extends object, AdditionalContext extends AllowableAdditionalContext> {
...
export interface ContextualAfterGenerator {
    <ExistingContext extends object>(ctxProm: Promise<ExistingContext>): ContextualAfterHook<ExistingContext>,
}

export const contextualAfterGeneratorIdentifier = identifier<ContextualAfterGenerator>();
const contextualAfterBinding = bind(contextualAfterGeneratorIdentifier).withDependencies([
    afterIdentifier
]).withProvider(after => {
    const contextualAfter: ContextualAfterGenerator = <ExistingContext extends object>(ctxProm: Promise<ExistingContext>) => {
...
const entrypointAfterBinding = bind(entrypointAfterIdentifier).withDependencies([
...
export const afterModule = createModule(
    contextualAfterBinding
).addBinding(entrypointAfterBinding);