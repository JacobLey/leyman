import { Context as MochaContext, Done, HookFunction, Test as MochaTest } from 'mocha';
import pDefer from 'p-defer';
import { type AllowableAdditionalContext, type MergeContext, mergeContexts } from './merge-context.js';
...
>(
    ...
    const asyncCb = async function(this: MochaContext) {
        acquireLock(this.runnable());
        const additional = await cb.apply(
            this, 
            [{ ...await existing }] as unknown as [ExistingContext, Done]
        );
        deferred.resolve(additional);
    };
...
> => {
    ...
    const doneCb = function(this: MochaContext, done: Done) {
        ...
        void Promise.resolve().then(async () => {
            ...
            try {
                ...
                const deferred = pDefer<MergeContext<ExistingContext, Awaited<AdditionalContext>>>();
    ...
    const asyncCb = async function(this: MochaContext) {
        ...
        const additional = await cb.apply(
            this, 
            [{ ...existing }] as unknown as [ExistingContext, Done]
        );
