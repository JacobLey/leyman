import type { Context as MochaContext, Done, Test as MochaTest, ExclusiveTestFunction } from 'mocha';
import { acquireLock, checkLock } from './execution-lock.js';

export type ValidDoneReturnTypes = undefined | null | 0 | 0n | false | '' | void;
 ...
 */
export const wrapTestWithContext = <
...
export interface GenericContextualTest {
    ...
    (
        name: string,
        fn: (this: MochaContext, ctx: object) => void
    ): MochaTest;
}
...
export const wrapTestWithEntrypoint = (
    ...
    return (title, cb): MochaTest => {
        ...
        return contextualTest(title, function(ignore, done) {
            return (cb as (this: MochaContext, done: Done) => ValidDoneReturnTypes).call(this, done);
        });
    };
};